  ...
  methods:{
    ...
    submitPaper(){
      ...
      for(let i=0;i<this.questionList.length;i++) {
          questionIds.push(this.questionList[i].questionId)
          switch(this.questionList[i].type)
          {
            case 4://填空
            case 5:
            case 6:
              answer.push(this.questionList[i].answer);
              break;
            case 1://选择
              answer.push(this.transformChar[this.questionList[i].answer]);
              break;
            case 3://判断
      ...
      httpServer({
        ...
        stuId : parseInt(localStorage.getItem('userId')),
  ...
  created(){
    ...
    if(true) {
      httpServer({
        ...
        paperId : parseInt(this.$route.params.paperId)
      })
      .then((res)=>{
